...
```yml
  Pyromaniac:
    ...
    Type: LEVEL_KIT
    Required-Level: 30
    Permission-To-Unlock: thebridge.kit.pyromaniac
    Start-Equipment:
      Inventory:
...
# REMEMBER to add thebridge.join.* permission to allow access to all random arenas!
Enabled: true
...
# TheBridge plugin
#
# Type here TheBridge server names that are hooked to Bungee Cord
Bungee-Server-Names:
  - "TB01"
```
